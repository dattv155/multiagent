...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        "*** CODE ***"
        define_lose = float('-inf')
        define_win = float('inf')

        if successorGameState.isWin():
            return define_win

        nearest_food_distance = min(manhattanDistance(food, newPos) for food in newFood.asList())
        ...
        return successorGameState.getScore() + 1.0 / nearest_food_distance
